...
class TemperamentTest
{
    ...
    public function new()
    {
        baseNote = Pitch.fromMidiNote(69);
        basePitch = 440.0;
    }
    ...
    public function testEqualTemperament()
    {
        ...
        return assert(Math.abs(temperament.pitchFromNote(Pitch.fromMidiNote(76)) - 659.26) < 0.01);
    }
    ...
    public function testArbitrary()
    {
        ...
        return assert(Math.abs(temperament.pitchFromNote(Pitch.fromMidiNote(62)) - 294.34) < 0.01);
    }

    public function testPythagorean()
    {
        var temperament = new Pythagorean(baseNote, basePitch);
        return assert(temperament.pitchFromNote(Pitch.fromMidiNote(64)) == 330.0);
    }

    public function testJust()
    {
        var temperament = new Just(baseNote, basePitch);
        return assert(temperament.pitchFromNote(Pitch.fromMidiNote(73)) == 550.0);
    }
